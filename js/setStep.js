...
function SetStep(arg, steps, cube) {
    this.body = document.body;
    this.opt = {
        show: false,
        content: '.stepCont',
...
//初始化 生成页面的进度条和按钮
SetStep.prototype.init = function (arg, cube) {
    var _that=this;
    extend(this.opt,arg);
    ...
    var stepP=$("<div class='ystep-progress'>"+
            ...
            "</div>");
    var stepButtonHtml =$( "<div class='step-button' id='div3'><button type='button' class='btn btn-default prevBtn' id='prevBtn' class='prevBtn'>　←　</button>"+
                        "<button type='button' class='btn btn-default nextBtn' id='nextBtn' class='nextBtn'>　→　</button></div>");
    ...
    this.setProgress(this.stepContainer,this.opt.curStep,this.opt.stepCounts, cube)
    //判断参数 是否显示按钮 并绑定点击事件
    if(this.opt.showBtn){
        ...
        this.prevBtn.on('click',function(){
            ...
            if($(_that).attr('disabled')||_that.opt.animating){
                ...
                _that.opt.curStep--;
                _that.setProgress(_that.stepContainer, _that.opt.curStep, _that.opt.stepCounts, cube, -1)
            }
        })
    ...
    if(this.opt.clickAble){
        stepsHtml.find('li').on('click',function(){
            ...
            _that.setProgress(_that.stepContainer,_that.opt.curStep,_that.opt.stepCounts, cube, 0)//注意这里方向暂时设为0了
        })
    }
...
//设置进度条
SetStep.prototype.setProgress = function (n, curIndex, stepsLen, cube, direction) {
      var _that=this;
        //获取当前容器下所有的步骤
        var $steps = $(n).find("li");
        ...
        if(1<=curIndex && curIndex<=$steps.length){
          ...
          $progress.animate({
            ...
            speed: 1000,
              ...
              _that.checkPage(_that.pageCont, _that.opt.curStep, _that.opt.stepCounts, cube, direction)
...
//改变 分页显示
SetStep.prototype.checkPage = function (pageCont, curStep, steps, cube, direction) {
    var t = this.opt.steps;
    for (var i = 1; i <= steps; i++) {
        if(i === curStep){
            if (i == 1 && direction == 1) break;
            if (direction == 1) {   //正向 下一步
                //alert(t[i - 1].toString());
