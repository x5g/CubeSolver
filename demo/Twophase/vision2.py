...
def getcolor(p):
    ...
    mh, ms, mv = median
    if ms <= Twophase.vision_params.sat_W and mv >= Twophase.vision_params.val_W:
        return median, 'white'
    elif Twophase.vision_params.orange_L <= mh < Twophase.vision_params.orange_H:
        return median, 'orange'
    elif Twophase.vision_params.orange_H <= mh < Twophase.vision_params.yellow_H:
        return median, 'yellow'
    elif Twophase.vision_params.yellow_H <= mh < Twophase.vision_params.green_H:
        if ms < 150:
            return median, 'white'  # green saturation is always higher
        else:
            return median, 'green'
    elif Twophase.vision_params.green_H <= mh < Twophase.vision_params.blue_H:
        if ms < 150:
...
def find_squares(bgrcap, n):
    ...
    for y in range(border, height - border, sz):
        for x in range(border, width - border, sz):
            ...
            delta = Twophase.vision_params.delta_C

            # if sigma is small enough define a mask on the 3x3 square with the grid square in it's center
            if sigma < Twophase.vision_params.sigma_W:
                rect3x3 = hsv[y - 1 * sz:y + 2 * sz, x - 1 * sz:x + 2 * sz]
                mask = cv2.inRange(rect3x3, (0, 0, Twophase.vision_params.val_W),
                                   (255, Twophase.vision_params.sat_W, 255))
            ...
            if sigma < Twophase.vision_params.sigma_C:
    ...
    black_mask = cv2.inRange(bgrcap, (0, 0, 0), (Twophase.vision_params.rgb_L, Twophase.vision_params.rgb_L, Twophase.vision_params.rgb_L))
...
def grab_colors():
    ...
    while 1:
        ...
        Twophase.vision_params.face_hsv, Twophase.vision_params.face_col = getcolors(cf, ef, acf, aef, m)
