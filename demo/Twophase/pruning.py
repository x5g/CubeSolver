...
import Twophase.defs
import Twophase.enums
...
def create_phase1_prun_table():
    ...
    total = Twophase.defs.N_FLIPSLICE_CLASS * Twophase.defs.N_TWIST
    fname = "phase1_prun"
    if not path.isfile(fname):
        ...
        cc = cb.CubieCube()
        fs_sym = ar.array('H', [0] * Twophase.defs.N_FLIPSLICE_CLASS)
        for i in range(Twophase.defs.N_FLIPSLICE_CLASS):
            if (i + 1) % 1000 == 0:
                print('.', end='', flush=True)
            rep = sy.flipslice_rep[i]
            cc.set_slice(rep // Twophase.defs.N_FLIP)
            cc.set_flip(rep % Twophase.defs.N_FLIP)

            for s in range(Twophase.defs.N_SYM_D4h):
                ...
                if ss.get_slice() == rep // Twophase.defs.N_FLIP and ss.get_flip() == rep % Twophase.defs.N_FLIP:
                    fs_sym[i] |= 1 << s
        print()
        # ##################################################################################################################
        ...
        twist = 0
        set_flipslice_twist_depth3(Twophase.defs.N_TWIST * fs_classidx + twist, 0)
        done = 1
        depth = 0
        backsearch = False
        ...
        while done != total:
            ...
            if depth < 8:
                ...
                mult = 1
            idx = 0
            for fs_classidx in range(Twophase.defs.N_FLIPSLICE_CLASS):
                ...
                twist = 0
                while twist < Twophase.defs.N_TWIST:
                    ...
                    if not backsearch and idx % 16 == 0 and flipslice_twist_depth3[idx // 16] == 0xffffffff \
                            and twist < Twophase.defs.N_TWIST - 16:
                        twist += 16
                        idx += 16
                        continue
                    ####################################################################################################
...
def create_phase2_prun_table():
    ...
    total = Twophase.defs.N_CORNERS_CLASS * Twophase.defs.N_UD_EDGES
    fname = "phase2_prun"
    ...
    if not path.isfile(fname):
        ...
        cc = cb.CubieCube()
        c_sym = ar.array('H', [0] * Twophase.defs.N_CORNERS_CLASS)
        for i in range(Twophase.defs.N_CORNERS_CLASS):
            if (i + 1) % 1000 == 0:
                print('.', end='', flush=True)
            rep = sy.corner_rep[i]
            cc.set_corners(rep)
        ...
        ud_edge = 0
        set_corners_ud_edges_depth3(Twophase.defs.N_UD_EDGES * c_classidx + ud_edge, 0)
        done = 1
        depth = 0
        print('depth:', depth, 'done: ' + str(done) + '/' + str(total))
        while depth < 10:  # we fill the table only do depth 9 + 1
            ...
            if depth > 9:
                mult = 1
            for c_classidx in range(Twophase.defs.N_CORNERS_CLASS):
                ...
                ud_edge = 0
                while ud_edge < Twophase.defs.N_UD_EDGES:

                    # ################ if table entries are not populated, this is very fast: ##########################
                    if idx % 16 == 0 and corners_ud_edges_depth3[idx // 16] == 0xffffffff \
                            and ud_edge < Twophase.defs.N_UD_EDGES - 16:
                        ud_edge += 16
                        idx += 16
                        continue
                    ####################################################################################################
                    ...
                    if get_corners_ud_edges_depth3(idx) == depth3:
                        ...
                        for m in (Twophase.enums.Move.U1, Twophase.enums.Move.U2, Twophase.enums.Move.U3, Twophase.enums.Move.R2, Twophase.enums.Move.F2,
                                  Twophase.enums.Move.D1, Twophase.enums.Move.D2, Twophase.enums.Move.D3, Twophase.enums.Move.L2, Twophase.enums.Move.B2):
...
def create_phase2_cornsliceprun_table():
    ...
    if not path.isfile(fname):
        print("creating " + fname + " table...")
        cornslice_depth = ar.array('b', [-1] * (Twophase.defs.N_CORNERS * Twophase.defs.N_PERM_4))
        corners = 0  # values for solved phase 2
        slice_ = 0
        cornslice_depth[Twophase.defs.N_PERM_4 * corners + slice_] = 0
        done = 1
        depth = 0
        idx = 0
        while done != Twophase.defs.N_CORNERS * Twophase.defs.N_PERM_4:
            for corners in range(Twophase.defs.N_CORNERS):
                for slice_ in range(Twophase.defs.N_PERM_4):
                    if cornslice_depth[Twophase.defs.N_PERM_4 * corners + slice_] == depth:
                        for m in (Twophase.enums.Move.U1, Twophase.enums.Move.U2, Twophase.enums.Move.U3, Twophase.enums.Move.R2, Twophase.enums.Move.F2,
                                  Twophase.enums.Move.D1, Twophase.enums.Move.D2, Twophase.enums.Move.D3, Twophase.enums.Move.L2, Twophase.enums.Move.B2):
                            corners1 = mv.corners_move[18 * corners + m]
                            slice_1 = mv.slice_sorted_move[18 * slice_ + m]
                            idx1 = Twophase.defs.N_PERM_4 * corners1 + slice_1
        ...
        cornslice_depth.fromfile(fh, Twophase.defs.N_CORNERS * Twophase.defs.N_PERM_4)
    fh.close()
