...
function SetStep(arg, steps, cube, arr) {
    this.body = document.body;
    this.opt = {
        show: false,
        content: '.stepCont',
        ...
        steps: steps,
        //pageClass:'',//分页的类或则id
        stepCounts: steps.length,//总共的步骤数
        curStep: 1,//当前显示第几页
        animating: false,
        showBtn: true,//是否生成上一步下一步操作按钮
        clickAble: true,//是否可以通过点击进度条的节点操作进度
    ...
    this.init(arg, cube, arr)
}



//初始化 生成页面的进度条和按钮
SetStep.prototype.init = function (arg, cube, arr) {
    var _that = this;
    extend(this.opt, arg);
    ...
    var stepP = $("<div class='ystep-progress'>" +
            ...
            "</div>");
    var stepButtonHtml = $("<div class='step-button' id='div3'>" +
                           "<button type='button' class='btn btn-default prevBtn' id='prevBtn' class='prevBtn'>　←　</button>" +
                           "<button type='button' class='btn btn-default nextBtn' id='nextBtn' class='nextBtn'>　→　</button>" +
                           "</div>");
    ...
    for (var i = 0; i < this.opt.stepCounts; i++) {
        var stepLabel = $("<label></label>");

        if (i == 0) {
            stepLabel.text(this.opt.steps[i]).addClass('');
            var _s = $(stepDisc).append(stepLabel);
            //var _s = $(stepDisc).text(this.opt.steps[i]).addClass('')
        } else {
            stepLabel.text(this.opt.steps[i]);
            var _s = $(stepDisc).append(stepLabel);
    ...
    stepsHtml.find('label').css('marginLeft', -w_li * 2);
    ...
    this.setProgress(this.stepContainer, this.opt.curStep, this.opt.stepCounts, cube, arr)
    //判断参数 是否显示按钮 并绑定点击事件
    if (this.opt.showBtn) {
        ...
        this.prevBtn.on('click', function () {
            ...
            if ($(_that).attr('disabled') || _that.opt.animating) {
                ...
                _that.opt.curStep--;
                _that.setProgress(_that.stepContainer, _that.opt.curStep, _that.opt.stepCounts, cube, -1, arr)
            }
        })
    ...
    if (this.opt.clickAble) {
        stepsHtml.find('li').on('click', function () {
            ...
            _that.setProgress(_that.stepContainer, _that.opt.curStep, _that.opt.stepCounts, cube, 0, arr)//注意这里方向暂时设为0了
        })
    }
...
//设置进度条
SetStep.prototype.setProgress = function (n, curIndex, stepsLen, cube, direction, arr) {
    var _that = this;
    //获取当前容器下所有的步骤
    var $steps = $(n).find("li");
    ...
    if (1 <= curIndex && curIndex <= $steps.length) {
        ...
        $progress.animate({
            ...
            speed: 1000,
                ...
                _that.checkPage(_that.pageCont, _that.opt.curStep, _that.opt.stepCounts, cube, direction, arr)
...
//改变 分页显示
SetStep.prototype.checkPage = function (pageCont, curStep, steps, cube, direction, arr) {
    var t = this.opt.steps;
    for (var i = 1; i <= steps; i++) {
        if (i === curStep) {
            if (i == 1 && direction == 1) break;
            if (direction == 1) {   //正向 下一步
                cube.turn3s(t[i - 1].toString());
            }
            else if (direction == -1) {
                if (t[i].toString().length == 1) {
                    cube.turn3s(t[i].toString() + '\'');
                }
                else {
                    if (t[i][1].toString() == '\'') {
                        cube.turn3s(t[i][0].toString());
                    }
                    else {
                        cube.turn3s(t[i].toString());
                    }
