...
class SolverThread(thr.Thread):
    ...
    def search(self, flip, twist, slice_sorted, dist, togo_phase1):
        ...
        if togo_phase1 == 0:  # phase 1 solved
            ...
            ud_edges = Twophase.coord.u_edges_plus_d_edges_to_ud_edges[24 * u_edges + d_edges % 24]
    ...
    def run(self):
        ...
        if self.rot == 0:  # no rotation
            cb = Twophase.cubie.CubieCube(self.cb_cube.cp, self.cb_cube.co, self.cb_cube.ep, self.cb_cube.eo)
        elif self.rot == 1:  # conjugation by 120° rotation
            cb = Twophase.cubie.CubieCube(sy.symCube[32].cp, sy.symCube[32].co, sy.symCube[32].ep, sy.symCube[32].eo)
            cb.multiply(self.cb_cube)
            cb.multiply(sy.symCube[16])
        elif self.rot == 2:  # conjugation by 240° rotation
            cb = Twophase.cubie.CubieCube(sy.symCube[16].cp, sy.symCube[16].co, sy.symCube[16].ep, sy.symCube[16].eo)
            cb.multiply(self.cb_cube)
            cb.multiply(sy.symCube[32])
        if self.inv == 1:  # invert cube
            tmp = Twophase.cubie.CubieCube()
            cb.inv_cubie_cube(tmp)
            cb = tmp

        self.co_cube = Twophase.coord.CoordCube(cb)  # the rotated/inverted cube in coordinate representation
