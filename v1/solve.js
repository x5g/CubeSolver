...
// combine:
// var ep_min      = new Uint16Array(E_PRM);
// var ep_min_op   = new Uint8Array(E_PRM);
// var ept_op_idx  = new Int16Array(E_PRM);

var epi = new Int16Array(E_PRM*3);  // 102 KB
...
function populate_epi()
{
  for (var i=0, j=0; i < E_PRM; i++, j+=3) {
    epi[j] = ep_min[i];
    epi[j+1] = ep_min_op[i];
    epi[j+2] = ept_op_idx[i];
  }
}
